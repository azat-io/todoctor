...
async fn test_primary_keyword_found() {
    ...
    assert_eq!(
        identify_todo_comment(comment, None, None),
        Some("TODO".to_string())
    );
}
...
async fn test_secondary_keyword_not_found_in_middle() {
    ...
    assert_eq!(identify_todo_comment(comment, None, None), None);
}
...
#[tokio::test]
async fn test_keywords_include() {
    let comment = "// eslint-disable-next-line no-console";
    assert_eq!(
        identify_todo_comment(
            comment,
            Some(&["eslint-disable-next-line"]),
            None,
        ),
        Some("eslint-disable-next-line".to_string())
    );
}

#[tokio::test]
async fn test_keywords_primary_exclude() {
    let comment = "// todo: This is a test comment.";
    assert_eq!(identify_todo_comment(comment, None, Some(&["TODO"])), None);
}

#[tokio::test]
async fn test_keywords_secondary_exclude() {
    let comment = "// IDEA: This is an idea comment.";
    assert_eq!(identify_todo_comment(comment, None, Some(&["IDEA"])), None);
}
